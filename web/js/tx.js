        ...
        for (var hash in inputs) {
            ...
            for (var index in inputs[hash]) {
                if (!inputs[hash].hasOwnProperty(index))
                    continue;
//console.log('scr is '+inputs[hash][index].script+'\n');
...
//console.debug('script is '+JSON.stringify(connectedScript,'','  ')+'\n')
...
// add this so that we know what address was used and how much it had on it
                if (sendTx.ins[i].address != undefined){
                  r['in'][i].address = sendTx.ins[i].address
                }
                if (sendTx.ins[i].value != undefined){
                  r['in'][i].value = sendTx.ins[i].value
                }
            }
        }
...
// add this so that we know what address was used and how much it had on it
            if (sendTx.outs[i].address != undefined){
              r['out'][i].address = sendTx.outs[i].address
            }
        }
