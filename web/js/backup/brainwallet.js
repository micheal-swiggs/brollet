...
// get a list of unspent transaction we can send from; make sure the from address is in also a to address
...
$('#debugKeys').val(JSON.stringify(Keys, '', '  '))
    ...
    function homeUpdateBalance(){
        ...
        s = "<img src='img/wait.gif' border=0 align=left>"
...
the above message will already be known from the EnigmaWallet info.
...
be in the senders EnigmaWallet.
...
*/
    function homeMakeEmailTx(){
      var to, from, amount, date, message, rawCode1, rand, key, bita
      to = removeWhiteSpace($('#homeSendToEmail').val())
...
// pick a random string
      rand = randomHex32()
// create the key
      key = hex2hexKey(hash256(rawCode1+rand))
...
// if the home wallet is still open, warn and close it before opening new wallet
      ...
      if (UI.walletType == undefined){ UI.walletType = 'enigma' }
      if (UI.walletType == 'enigma'){
...
//        $('#homeSendShowBtn').click(homeSendShowTx);
        ...
        // advanced
        $('#advancedEnigmaBtn').click(advancedEnigmaBtn);
